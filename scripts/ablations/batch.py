"""Ablation study on the batch size in the watermarking method"""
import os
...
def run(file: str, batch_size: int):
    ...
    config = ExperimentConfig.from_yaml({"cfg_name": cfg_name,
                                        #  'QueryModelExtraction', 'ModelExtractionCF', 'DualCFAttack'
                                         ...
                                         "save_cfg": False})
    config.watermark_conifg['batch_size'] = batch_size
    logger = config.logger
    shutil.rmtree(logger.path)
    setattr(logger, 'path', 
            Path(f'logs/ablation/{config.logger.name}/{config.attack_method_cls}/batch_size={batch_size}'))
...
constraints = {
    'data': ['dummy'],
    'method_name': ['GrowingSphere', 'DiverseCF'] #['CCHVAE'] #, 'GrowingSphere'],
}
...
for root, dirs, files in os.walk(config_dir):
    for file in files:
        if file.endswith(".yaml"):
            for n_ensembles in [2, 4, 8, 16, 32, 64, 128]:
