"""Ablation study on using training data in the watermarking method"""
import os
...
def run(file: str):
    ...
    config = ExperimentConfig.from_yaml({"cfg_name": cfg_name,
                                        #  'QueryModelExtraction', 'ModelExtractionCF', 'DualCFAttack'
                                         ...
                                         "save_cfg": False})
    config.watermark_conifg['use_train_data'] = False
    logger = config.logger
    shutil.rmtree(logger.path)
    setattr(logger, 'path', 
            Path(f'logs/ablation/{config.logger.name}/{config.attack_method_cls}/no_train_data'))
