"""Ablation study on the regularization term in the watermarking method"""
import os
...
def run(file: str, lambdas: float):
    ...
    config = ExperimentConfig.from_yaml({"cfg_name": cfg_name,
                                        #  'QueryModelExtraction', 'ModelExtractionCF', 'DualCFAttack'
                                         "attack_method_cls": 'DualCFAttack',
                                         "use_default_exps": True,
                                         "save_cfg": False})
    config.watermark_conifg['lambdas'] = (1.0 + lambdas, 1.0, lambdas)
    logger = config.logger
    shutil.rmtree(logger.path)
    setattr(logger, 'path', 
            Path(f'logs/ablation/{config.logger.name}/{config.attack_method_cls}/lambda={lambdas}'))
